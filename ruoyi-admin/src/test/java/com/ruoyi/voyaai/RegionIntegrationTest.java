...
class RegionIntegrationTest {
    ...
    @BeforeEach void reset() throws Exception {
        ...
        ProvinceCreateDTO province = new ProvinceCreateDTO();
        province.setCountryId(1L); province.setName("四川");
        provinces.create(province, "admin");
    }

    private CityCreateDTO city(String name) {
    ...
    @Test void duplicateIncludesDeletedNamesAndBlankCodesAreNull() {
        ...
        for (String name : new String[]{"日本", "韩国"}) {
            CountryCreateDTO country = new CountryCreateDTO(); country.setName(name); country.setCode(" ");
            countries.create(country, "admin");
        }
    ...
    @Test void invalidParentStatusAndCoordinatesAreRejected() {
        CityCreateDTO dto = city("成都"); dto.setLatitude(new BigDecimal("91"));
    ...
    @Test void dtoBoundaryAndStatusValidation() throws Exception {
        assertThrows(NoSuchMethodException.class, () -> CityCreateDTO.class.getMethod("setId", Long.class));
        for (String field : new String[]{"setCreateBy", "setUpdateBy", "setDelFlag"}) {
            assertThrows(NoSuchMethodException.class, () -> CityCreateDTO.class.getMethod(field, String.class));
        }
    ...
    @Test void countryAndProvinceCrudAndPagination() {
        ...
        ProvinceUpdateDTO province = new ProvinceUpdateDTO();
